...
class RpcClient {
  ...
  rpc(request) {
    ...
    if (process.env.SBITINFO_ENV === 'debug') {
        this.#log.error('RPC Request: ', request)
    }
      ...
      let req = this.#protocol.request(options, res => {
        ...
        res.on('end', () => {
          if (res.statusCode === 401) {
            reject(new Error(`Sbit JSON-RPC: Connection Rejected: 401 Unauthorized`))
          } else if (res.statusCode === 403) {
            reject(new Error(`Sbit JSON-RPC: Connection Rejected: 403 Forbidden`))
          } else if (res.statusCode === 500 && buffer === 'Work queue depth exceeded') {
            let exceededError = new Error(`Sbit JSON-RPC: ${buffer}`)
            ...
            try {
              ...
              reject(new Error(`Sbit JSON-RPC: Error Parsing JSON: ${err.message}`))
            }
          }
        })
      })
      req.on('error', err => reject(new Error(`Sbit JSON-RPC: Request Error: ${err.message}`)))
