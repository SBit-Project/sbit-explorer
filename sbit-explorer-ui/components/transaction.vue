<template>
  <div class="columns is-multiline transaction-item">
          ...
          <div class="output-script">
            ...
            <div v-if="output.scriptPubKey.asm">
              ...
              <code class="value" :class="{script: contractInfo[index]}"
                ...
                -->{{ output.scriptPubKey.asm | sbit-script }}<!--
              --></code>
            </div>
    ...
    <template v-for="({addressHex, name, symbol, decimals, from, fromHex, to, toHex, value}, index) in src20TokenTransfers">
      <div class="column is-full flex-full"></div>
      <AttributeInjector
    ...
    <template v-for="({addressHex, name, symbol, from, fromHex, to, toHex, tokenId}, index) in src721TokenTransfers">
      <div class="column is-full flex-full"></div>
      <AttributeInjector
        class="column collapse token-transfer-list"
        :class="{
    ...
    <div class="column is-full has-text-right collapse-bottom" v-if="fees !== '0'">
      <template v-if="fees > 0">
        {{ $t('transaction.fee') }} <span class="amount fee">{{ fees | sbit }} MRX</span>
      </template>
      <template v-else-if="fees < 0">
        {{ $t('transaction.reward') }} <span class="amount fee">{{ -fees | sbit }} MRX</span>
      </template>
    </div>
  </div>
</template>
