...
class TransactionService extends Service {
  async getTransaction(id) {
    ...
    const {Address: RawAddress} = this.app.sbitinfo.lib
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.sbitinfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.sbitinfo.rpc(this.app.config.sbitinfo.rpc)
  ...
  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.sbitinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.sbitinfo.lib
  ...
  async transformSRC20Transfers(outputs) {
    const TransferABI = this.app.sbitinfo.lib.Solidity.src20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
  ...
  async transformSRC20UnconfirmedTransfers(outputs) {
    const {OutputScript, Solidity} = this.app.sbitinfo.lib
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.sbitinfo.lib
    const {
